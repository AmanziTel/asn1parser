...
public enum ReservedWord implements IToken {
	BEGIN("BEGIN"), BIT_STRING("BIT STRING"), BOOLEAN("BOOLEAN"), CHOICE(
			"CHOICE"), CONTAINING("CONTAINING"), DEFINITIONS_AUTOMATIC_TAGS(
			"DEFINITIONS AUTOMATIC TAGS"), END("END"), ENUMERATED("ENUMERATED"), FROM(
			"FROM"), IMPORTS("IMPORTS"), INTEGER("INTEGER"), NULL("NULL"), OCTET_STRING(
			"OCTET STRING"), OF("OF"), OPTIONAL("OPTIONAL"), SEQUENCE(
			"SEQUENCE"), SEQUENCE_OF("SEQUENCE_OF"), SIZE("SIZE");
	...
	public static ReservedWord[] getPossibleTokens(char character) {
		...
		if (result == null) {
			...
			for (ReservedWord singleToken : values()) {
				if (ArrayUtils.contains(singleToken.getTokenText()
